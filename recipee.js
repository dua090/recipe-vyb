    ...
    try {
        ...
        if (jsonMatch) {
            try {
                ...
                console.warn(`Failed to parse Gemini nutrition JSON for ${ingredientName}:`, e.message, text);
                return estimateBasicNutrition(ingredientName, weightInGrams);
            }
        }

        throw new Error('Could not parse nutrition estimate from Gemini');
    } catch (error) {
...
function estimateBasicNutrition(ingredientName, weightInGrams) {
    ...
    if (name.includes('oil') || name.includes('ghee') || name.includes('butter')) {
        ...
        nutrition = { calories: 23, protein: 2, carbs: 4, fat: 0.5, fiber: 2 };
    }
...
app.post('/estimate', async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({ error: 'Failed to estimate nutrition', message: error.message });
    }
});

// Default route
app.get('/', (req, res) => {
    res.send('Indian Dish Nutrition Estimator API. Send a POST request to /estimate with a JSON body containing { "dishName": "your dish" }');
});
